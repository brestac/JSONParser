...
enum SKIP : uint8_t { NONE = 0, END = 1, STOP = 2 };
struct ParseResult;
NAMESPACE_JSON_END
...
struct JSONCallbackObject {
  ...
  void run(const JSONValue& value) {
    if (callback) {
      JSON_DEBUG_INFO(
          "JSONCallbackObject running callback with key %.*s _array_index=%d\n",
  ...
  void push() { this->key._array_index.push(this->key.getArrayIndex()); }

  void pop() { this->key._array_index.pop(); }
};
